...
class QdrantRepo:
    ...
    def __init__(self):
        ...
        if QdrantRepo._client is None:
            try:
                # Try to connect with a short timeout so fallback triggers fast if offline
                QdrantRepo._client = QdrantClient(url=self.qdrant_url, timeout=3.0)
                # Test connection
                QdrantRepo._client.get_collections()
                logger.info(f"Successfully connected to Qdrant at {self.qdrant_url}")
            except Exception as e:
                logger.warning(f"Failed to connect to Qdrant at {self.qdrant_url}: {e}. Enabling in-memory fallback store.")
                self.use_fallback = True
                QdrantRepo._client = None
    ...
    def init_collection(self, collection_name: str, vector_size: int = 768):
        ...
        Default vector size is 768 (matching Google GenAI text-embedding-004).
        """
        if self.use_fallback:
            if collection_name not in QdrantRepo._fallback_db:
                QdrantRepo._fallback_db[collection_name] = []
            logger.info(f"[Fallback Store] Initialized collection: {collection_name}")
            return True

        try:
            collections_resp = QdrantRepo._client.get_collections()
            collection_names = [col.name for col in collections_resp.collections]
            
            if collection_name not in collection_names:
                QdrantRepo._client.create_collection(
    ...
    def upsert_chunks(self, collection_name: str, chunks: List[Dict[str, Any]]):
        ...
        Expects a list of dictionaries with structure:
        {
            ...
            "payload": dict (metadata, text, etc.)
        }
        """
        ...
        if self.use_fallback:
            ...
            existing = QdrantRepo._fallback_db[collection_name]
            # Remove existing matching IDs to prevent duplicates
            new_ids = {c["id"] for c in chunks}
            existing = [item for item in existing if item["id"] not in new_ids]
            existing.extend(chunks)
            QdrantRepo._fallback_db[collection_name] = existing
        ...
        try:
            ...
            if collection_name not in QdrantRepo._fallback_db:
                QdrantRepo._fallback_db[collection_name] = []
            QdrantRepo._fallback_db[collection_name].extend(chunks)
            return False
    ...
    def search_chunks(self, collection_name: str, query_vector: List[float], limit: int = 5, filter_course: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Searches for the closest chunks. Returns structured dictionary payloads.
        """
        self.init_collection(collection_name)

        if self.use_fallback:
            # Local cosine similarity lookup
            import math
            ...
            all_points = QdrantRepo._fallback_db.get(collection_name, [])
        ...
        try:
            # Setup filter if filter_course is set
            query_filter = None
            if filter_course:
            ...
            results = QdrantRepo._client.search(
                collection_name=collection_name,
